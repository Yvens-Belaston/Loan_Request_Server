...
A ressource that send the list of loan requests
grouped by status and ordered by submission date

The project **loanRequestList** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/loanRequestList.git
cd loanRequestList/
mvn package
----

The build should produce a zip archive under the `target/` folder named `loanRequestList-0.0.1.zip`
...
Take the built artifact `loanRequestList-0.0.1.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
